...
rng_x   <- range(df_ps_detail_max$tm)
max_auc <- approxfun(df_ps_detail_max$tm, df_ps_detail_max$qrt4) %>% 
  integrate(lower = rng_x[1], upper = rng_x[2])

df_ps_detail_max %>%
  ggplot(mapping = aes(x = tm, y = qrt4)) + 
  geom_hline(yintercept = 0, color = gray(.5)) + 
  geom_area(alpha = .3, color = "black", linewidth = 1) + 
  scale_x_continuous(breaks = 0:24, minor_breaks = NULL) + 
  annotation_custom(
    grob = grid::textGrob(
      label = sprintf(fmt = "%.2f kWh", max_auc$value/1000),
      x = 0.5, y = 0.5, just = "center", gp = grid::gpar(cex = 3, alpha = .5)
    )
  ) +
  labs(
    title = "Maximal production of balcony power station",
    x = "Hour",
    y = "Watt"
  ) + 
  theme_minimal()

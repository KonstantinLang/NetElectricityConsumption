...
```{r viz}
df_all %>% 
  mutate(kwh = if_else(as.integer(cat) == 2, -1 * kwh, kwh)) %>% 
  ggplot(mapping = aes(x = dt, y = kwh, group = cat, fill = cat)) + 
  geom_col() + 
  geom_text(
    aes(label = sprintf(fmt = "%.1f", abs(kwh))),
  ...
  scale_y_continuous(
    breaks = -9:9,
    labels = \(x) if_else(x < 0, "", as.character(x)),
    minor_breaks = NULL,
    sec.axis = dup_axis(name = "net kWh usage", breaks = -9:0, labels = 9:0)
  ) + 

...
```{r data-prep}
source(file.path("scripts", "f_auc.R"))
...
df_ps_daily %>% 
  ...
  geom_hline(yintercept = avg_usage, col = "red") + 
  geom_text(
    mapping = aes(
      x = as.Date("2025-07-08"),
      y = avg_usage,
      label = "Average electricity usage 2024"
    ),
    data = ~slice(.x, 1), hjust = -.1, vjust = -.5, color = "red"
  ) +
  geom_col() + 
  geom_text(
...
```{r, fig.height = 2}
df_ps_daily %>% 
  ggplot(mapping = aes(x = prod, y = 0)) + 
  facet_wrap(~month) + 
  geom_boxplot(fill = "#00000000", coef = NULL) + 
  geom_jitter(width = 0, height = .2, alpha = .4) + 
  scale_x_continuous(breaks = 0:9, minor_breaks = NULL, limits = c(0, NA)) + 
  scale_y_continuous(breaks = NULL) + 
  labs(
    y = NULL,
    x = "kWh",
    title = "Boxplots of daily electricity production"
  ) +
